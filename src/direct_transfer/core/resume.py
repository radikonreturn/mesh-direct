...
from direct_transfer.core.protocol import TransferOffer, validate_filename
...
class PartialTransferStore:
    ...
    def __init__(
        self, receive_dir: str | Path, partial_root: str | Path | None = None
    ) -> None:
        self._receive_dir = Path(receive_dir)
        self._root = (
            Path(partial_root)
            if partial_root is not None
            else self._receive_dir / ".direct-transfer-partials"
        )
