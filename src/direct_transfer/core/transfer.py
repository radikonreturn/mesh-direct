...
Security:
    ...
    Each connection derives an independent key from Ed25519-authenticated ephemeral
    X25519 keys.
...
from direct_transfer.core.inbox import (
    InboxCapacityError,
    IncomingRequestManager,
    IncomingRequestStatus,
    IncomingTransferRequest,
)
from direct_transfer.core.lifecycle import is_terminal, set_status
from direct_transfer.core.models import (
    TERMINAL_TRANSFER_STATUSES,
    TransferDirection,
    TransferInfo,
    TransferStatus,
)
from direct_transfer.core.pairing import server_pair
...
from direct_transfer.core.resume import PartialTransferStore, commit_partial_file
...
from direct_transfer.core.trust import TrustedDevice, TrustStatus, TrustStore
...
class FileServer(threading.Thread):
    ...
    Protocol v3 authenticates the peer before reading encrypted offers or chunks.

    Usage:
        server = FileServer(identity=identity, trust_store=trust_store)
        server.start()
        server.shutdown()
    """
    ...
    def __init__(
        ...
        self._partials = PartialTransferStore(
            self._receive_dir, partial_root=partial_dir
        )
        self._identity = identity
        self._trust_store = trust_store
        if identity is None or trust_store is None:
            raise ValueError("Protocol v3 requires identity and trust store")
    ...
    def _receive_session(self, conn: socket.socket, peer_ip: str) -> None:
        ...
        try:
            ...
            initial_hello = _receive_json(conn)
            if initial_hello.get("type") == "pair_hello":
                paired = server_pair(
                    conn, self._identity, peer_ip, initial_hello, self._port
                )
                if self._on_pairing is not None:
                    self._on_pairing(paired)
                return
...
class FileClient:
    ...
    Protocol v3 authenticates each peer before encrypted offers and chunks.
    ...
    Usage:
        client = FileClient(identity=identity, trust_store=trust_store)
    ...
    def _assert_peer_protocol(
        ...
        """Fail before connecting when endpoint metadata reports incompatibility."""
    ...
    def _batch_worker_entry(
        ...
        try:
            self._batch_worker(
                peer_ip, filepaths, message, transfer_id, cancel_event, port
            )
        finally:
            with self._lock:
                self._cancel_events.pop(transfer_id, None)
    ...
    def _batch_worker(
        ...
        for attempt in range(MAX_RETRIES + 1):
            ...
            try:
                ...
                log.warning("Cryptographic tag verification failed for %s", peer_ip)
                for info in infos:
                    if not is_terminal(info.status):
                        set_status(
                            info,
    ...
    def _send_session(
        ...
        self._assert_peer_protocol(peer_ip, PROTOCOL_VERSION)

        sock = socket.socket(
            socket.AF_INET6 if ":" in peer_ip else socket.AF_INET,
            socket.SOCK_STREAM,
        )
        ...
        try:
            ...
            offer_frame = {
                ...
                "files": [
                    ...
                    for (path, size, sha), info in zip(file_meta, infos, strict=False)
                ],
            }
            if message:
    ...
    def _send_v3_payload(
        ...
        for (path, size, _sha), info, offered in zip(
            file_meta, infos, offer.files, strict=False
        ):
...
class SecureTransfer:
    ...
    Uses authenticated protocol v3 via FileServer and FileClient.
    """
    ...
    def __init__(
        ...
        self._server = FileServer(
            ...
            incoming_manager=self._incoming,
    ...
    def send_file(
        ...
        *,
        ...
        return self._client.send_batch(peer_ip, paths, message=message, port=port)
    ...
    def _persist_runtime_transfers(self) -> None:
        ...
        for transfer_id, raw_infos in grouped.items():
            ...
            record = TransferRecord(
                ...
                resumed=any(item.resume_offset > 0 for item in infos),
                resume_offset=sum(item.resume_offset for item in infos),
            )
            files = [
                TransferFileRecord(
                    id=None,
